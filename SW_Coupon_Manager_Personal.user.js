...
const SOURCE_URLS = [
  {name:'SWGT', url:'https://swgt.io/gamecodes'},
  {name:'SW-Teams', url:'https://sw-teams.ovh/codes'}
];
...
function extractCouponCodes(text){
  const upper=String(text||'').toUpperCase();
  const labelled=[...upper.matchAll(/\bCODE\s*:\s*([A-Z0-9]{7,32})\b/g)].map(m=>m[1]);
  const general=upper.match(/\b[A-Z0-9]{7,32}\b/g)||[];
  return [...new Set([...labelled,...general])].filter(code=>
    /[A-Z]/.test(code) && /\d/.test(code) && !/^\d+$/.test(code) && !CODE_BLACKLIST.has(code)
  );
}
...
function scanSource(source){
  return new Promise((resolve,reject)=>{
    GM_xmlhttpRequest({
      method:'GET', url:source.url+'?_='+Date.now(), timeout:15000,
      headers:{'Cache-Control':'no-cache'},
      onload:r=>{
        if(r.status<200||r.status>=400)return reject(new Error(source.name+' HTTP '+r.status));
        try{
          const doc=new DOMParser().parseFromString(r.responseText,'text/html');
          const codes=extractCouponCodes(doc.body?.innerText||'');
          if(!codes.length) throw new Error('쿠폰 코드를 찾지 못했습니다.');
          resolve({source:source.name,codes});
        }catch(e){reject(new Error(source.name+': '+String(e?.message||e)));}
      },
      onerror:()=>reject(new Error(source.name+' 접속 실패')),
      ontimeout:()=>reject(new Error(source.name+' 접속 시간 초과'))
    });
  });
}
...
async function doScan(){
  reloadDB();
  db.lastError='';
  db.lastMessage='SWGT + SW-Teams 스캔 중...';
  save(); render();
  const results=await Promise.allSettled(SOURCE_URLS.map(scanSource));
  const succeeded=results.filter(r=>r.status==='fulfilled').map(r=>r.value);
  const failed=results.filter(r=>r.status==='rejected').map(r=>String(r.reason?.message||r.reason));
  if(!succeeded.length){
    db.lastSources=[];
    db.lastError=failed.join(' · ')||'모든 쿠폰 소스 스캔 실패';
    db.lastMessage='스캔 실패';
    save(); render();
    throw new Error(db.lastError);
  }
    const codes=[...new Set(succeeded.flatMap(r=>r.codes))];
    ...
    db.scanCodes=codes;
    db.newCodes=newly;
    db.lastSources=succeeded.map(r=>r.source);
    db.lastScanAt=new Date().toLocaleString();
    db.knownCodes=[...new Set([...(db.knownCodes||[]),...codes])];
    db.lastError=failed.length ? failed.join(' · ') : '';
    db.lastMessage=`활성 ${codes.length}개 · ${db.lastSources.join(' + ')}`;
    save(); render();
    return codes;
}
...
function render(){
  ...
  p.innerHTML=`
    <div id="sw-head" style="padding:10px 12px;background:#1b1b1b;border-bottom:1px solid #333;cursor:move;display:flex;justify-content:space-between;align-items:center">
      <b style="font-size:15px">SW 쿠폰 매니저 v4.8</b><div style="display:flex;gap:6px">${badge}<button id="collapse">${db.ui.collapsed?'▼':'▲'}</button></div>
    </div>
